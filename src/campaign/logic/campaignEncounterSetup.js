...
export function campaignHand(deck, requiredId, seed, salt) {
  ...
  const required =
    requiredId == null ? null : deck.find((c) => c.id === requiredId);
  if (requiredId != null && !required)
    throw new Error('Carta garantita assente dal mazzo.');
  ...
  if (hand.length !== 5) throw new Error('Mano campagna incompleta.');
  return hand;
}
export function campaignFixedHands(mission, run, playerDeck, enemyDeck) {
  return {
    playerHand: campaignHand(
      playerDeck,
      NASCENTE_ID,
      run.seed,
      `${mission.id}:player`,
    ),
    enemyHand: campaignHand(
      enemyDeck,
      mission.signatureCardId ?? null,
      run.seed,
      `${mission.id}:enemy`,
    ),
  };
