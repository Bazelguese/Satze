...
import { CHOICE_STATES } from '../../game/eminence/eminenceChoiceView.js';
...
function EminenzaCard({ eminence, presence, accent, appearance, artX, artY, artZoom, artFocusX, artFocusY }) {
...
function EminenzaRailItem({
  ...
  return (
    ...
    >
      ...
      {selected && onConfirm && (
        <div className="em-rail-confirm-hint">Clicca di nuovo per confermare</div>
      )}
    </div>
  );
}
    ...
    <div className="em-rail">
      {options.map((option, i) => (
        <EminenzaRailItem
          ...
          onConfirm={locked ? null : onConfirm}
        />
      ))}
    </div>
  );
...
export function EminenzaZone({
  ...
  const locked = !isPlayer
    || choiceState === CHOICE_STATES.LOCKED_HIDDEN
    || choiceState === CHOICE_STATES.REVEALED;

  return (
    <div className={`em-layer em-layer-${side}${embedded ? ' em-layer-embedded' : ''}`}>
      ...
      >
        ...
        {choiceState === CHOICE_STATES.LOCKED_HIDDEN && isPlayer && (
