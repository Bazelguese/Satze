...
export function CampaignDialog({ title, onClose, children, kicker = 'RICOGNIZIONE' }) {
  ...
  return <div className="cs-dialog-shade" onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={ref} className="cs-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      <header><div><p className="cs-kicker">{kicker}</p><h2 id={titleId}>{title}</h2></div><button onClick={onClose}>Chiudi</button></header>
      {children}
    </section>
  </div>;
}
