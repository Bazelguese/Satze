...
function alternate(ctx) {
  ...
  restack(order, tShuf);
  const dealStart = tShuf + 640;

  if (typeof ctx.battlefieldDeal === 'function') {
    return ctx.battlefieldDeal(order, dealStart);
  }
  ...
  for (let k = 0; k < H; k++) {
    ...
    const discardId = order[H + k];
    after(t, () => {
      const s = handSlot(k, H, g);
      setCard(handId, { x: s.x, y: s.y, rot: s.rot, z: 60 + k * 2, flipped: true, scale: g.dealScale });
    });
    t += 250;
    after(t, () => setCard(discardId, {
      x: DISC.x + k * 0.6,
      y: DISC.y + k * 1.4,
      rot: (k - 2) * 7,
      z: 30 + k,
