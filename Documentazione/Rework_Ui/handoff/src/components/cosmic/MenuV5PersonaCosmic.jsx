...
function MenuV5PersonaCosmic() {
  ...
  const items = [
    { id: 'partita',      label: 'PARTITA LOCALE', sub: 'AVVIO',     accent: HEAT,      meta: 'CLIC · POI SCEGLI LA MODALITÀ' },
    { id: 'campagna',     label: 'CAMPAGNA',       sub: 'STORIA',    accent: ACCENT,    meta: 'SLOT SALVATAGGIO' },
    { id: 'multiplayer',  label: 'MULTIPLAYER',    sub: 'ONLINE',    accent: HEAT,      meta: 'LOBBY · BETA' },
    { id: 'mazzi',        label: 'GESTIONE MAZZI', sub: 'DECK',      accent: '#22d3ee', meta: 'LISTA / BUILDER' },
    { id: 'tutorial',     label: 'TUTORIAL',       sub: 'GUIDA',     accent: '#a78bfa', meta: 'PRIMI PASSI' },
    { id: 'galleria',     label: 'GALLERIA',       sub: 'ARCHIVIO',  accent: HEAT,      meta: 'CARTE · CAMPI' },
    { id: 'devtools',     label: 'STRUMENTI DEV',  sub: 'LAB',       accent: '#94a3b8', meta: 'CLIC · POI SCEGLI LO STRUMENTO' },
  ];
  ...
  return (
    <div style={{
      ...
      {/* === FLOATING ASHES / SPORES (cosmic embers) === */}
      <div style={{position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none', zIndex: 1}}>
        {Array.from({length: 26}).map((_, i) => {
          ...
          const size = 2 + (i % 4);
          const color = i % 3 === 0 ? HEAT : i % 3 === 1 ? ACCENT : '#a78bfa';
          return (
            <div key={i} style={{
      ...
      {/* === ARCANE SIGIL — bottom-left, slowly rotating rune-ring === */}
      <div style={{
      ...
      {/* === ENERGY ARCS — pulsing connectors behind the logo === */}
      <svg style={{position:'absolute', top: 80, left: 60, width: 360, height: 220, zIndex:1, pointerEvents:'none', opacity:0.4}}>
        <defs>
          <radialGradient id="arc-glow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor={HEAT} stopOpacity="0.8"/>
            <stop offset="100%" stopColor={ACCENT} stopOpacity="0"/>
          </radialGradient>
        </defs>
        {[0,1,2,3].map(i => (
          <circle key={i} cx={60 + i*80} cy={140 - (i%2)*60} r="2"
            fill={HEAT}
            style={{animation: `arc-pulse 2.4s ${i*0.3}s ease-in-out infinite`, transformOrigin: 'center'}}/>
        ))}
        <path d="M 60 140 Q 200 60 320 140 T 460 100"
          stroke={ACCENT} strokeWidth="0.6" fill="none" strokeDasharray="2 4"
          style={{animation: 'arc-shift 6s linear infinite'}}/>
      </svg>
...
function TelemetryLine() { return null; }

// Inject keyframes once
if (typeof document !== 'undefined' && !document.getElementById('cosmic-extra-kf')) {
  const s = document.createElement('style');
  s.id = 'cosmic-extra-kf';
  s.textContent = `
    ...
    @keyframes blood-drip {
      ...
      8%   { opacity: 1; }
      92%  { opacity: 1; transform: translateY(100vh) scaleY(1); }
...
export default MenuV5PersonaCosmic;
