/* ============================================================
   ...
   v0.2 — ENFASI SELETTIVA:
   Il testo ora è FERMO di default. Si anima solo ciò che marchi:
     *parola*        -> effetto FIRMA dell'armata su quella parola
     *frase intera!* -> tutta la frase animata (esclamazioni)
     {fx:parola}     -> override con un effetto specifico (es. {glitch:copiare})
   Retro-compat: passa emphasis:'all' a say() per animare tutta la riga
   con l'effetto firma (comportamento vecchio).
   ============================================================ */
(function (global) {
  'use strict';
  ...
  // ---- Parser markup enfasi ----
  // Ritorna array di token { ch, fx } dove fx === null significa "statico".
  //  *...*      -> fx = sigFx (effetto firma armata)
  //  {name:...} -> fx = name  (override)
  //  forceAll   -> ignora il markup, anima tutto con sigFx (retro-compat)
  function tokenize(text, sigFx, forceAll) {
    var out = [], i = 0, n = text.length, k, c;
    if (forceAll) {
      for (k = 0; k < n; k++) out.push({ ch: text[k], fx: sigFx });
      return out;
    }
    while (i < n) {
      c = text[i];
      if (c === '{') {
        ...
        if (close > -1 && colon > -1 && colon < close) {
          var fxName = text.slice(i + 1, colon).trim();
          var content = text.slice(colon + 1, close);
          for (k = 0; k < content.length; k++) out.push({ ch: content[k], fx: fxName });
          i = close + 1; continue;
        }
      }
      if (c === '*') {
        ...
        if (end > -1) {
          var seg = text.slice(i + 1, end);
          for (k = 0; k < seg.length; k++) out.push({ ch: seg[k], fx: sigFx });
          i = end + 1; continue;
        }
      }
  ...
  function makeChar(ch, idx, fx) {
    ...
    // statico: nessuna animazione, resta perfettamente leggibile
    if (!fx || fx === 'static') { sp.className = 'sd-c'; return sp; }
  ...
  function Box(opts) {
    ...
    this._raf = 0; this._toks = []; this._done = false; this._onDone = null;
  ...
  Box.prototype._type = function (text, sigFx, charMs, onDone, forceAll) {
    cancelAnimationFrame(this._raf);
    var toks = tokenize(text, sigFx, forceAll);
    this._toks = toks; this._done = false; this._onDone = onDone || null;
    var line = this.lineEl, arrow = this.arrowEl, self = this;
    line.textContent = '';
    arrow.style.display = 'none';
    var start = performance.now();
    function step(now) {
      var n = Math.min(toks.length, Math.floor((now - start) / charMs));
  ...
  Box.prototype.skip = function () {
    cancelAnimationFrame(this._raf);
    var line = this.lineEl, toks = this._toks || [];
  ...
  global.SatzeDialogue = { Box: Box, FONTS: FONTS, EFFECTS: EFFECTS, ARMY: ARMY, injectFonts: injectFonts, makeChar: makeChar, tokenize: tokenize };
})(window);
