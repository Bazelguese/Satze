...
function DeckSelectCosmic({
  ...
  const safeDecks = decks.length ? decks : [{
    id: '__empty', name: 'Nessun mazzo', fac: '—', sigil: '◇',
    cards: 0, lega: 0, pot: 0, dan: 0, win: 0, lead: '',
    warning: 'Nessun mazzo disponibile',
  }];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button style={{
          ...
          cursor: 'pointer',
        }} onClick={onBack}>← INDIETRO</button>
        ...
        <div style={{textAlign: 'center'}}>
          ...
          }}>{safeDecks.length} {safeDecks.length === 1 ? 'MAZZO' : 'MAZZI'} DISPONIBILI · SELEZIONA UN'ARMA</div>
        </div>
        ...
        <div style={{textAlign: 'right'}}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              color: '#06030a',
            }}>{opponent.sigil || 'X'}</div>
            <div>
              <div style={{
                fontFamily: 'Cinzel, serif', fontWeight: 700,
                fontSize: 14, letterSpacing: '0.15em',
                color: '#f5f3eb',
              }}>{(opponent.name || 'AVVERSARIO').toUpperCase()}</div>
              <div style={{fontSize: 9, color: HEAT, letterSpacing: '0.3em'}}>{(opponent.faction || '').toUpperCase()}{opponent.level != null ? ' · LIV ' + opponent.level : ''}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Prev arrow */}
          <button onClick={() => setActive((active - 1 + safeDecks.length) % safeDecks.length)}
          ...
          {/* Next arrow */}
          <button onClick={() => setActive((active + 1) % safeDecks.length)}
        ...
        <div style={{
            ...
            <div style={{
              ...
              {curveData.map((h, i) => (
                <div key={i} style={{flex: 1, display:'flex', flexDirection:'column', alignItems:'center', gap: 4}}>
                  <div style={{
                    width: '100%', height: (h / curveMax) * 56,
          ...
          <div style={{display: 'flex', gap: 8, marginBottom: 10}}>
            ...
            }} onClick={() => onPreviewDeck && onPreviewDeck(cur)}>👁 ANTEPRIMA</button>
            <button style={{
              flex: 1, padding: '10px',
              background: 'transparent',
              border: `1px solid ${VIOLET}88`,
            ...
            }} onClick={() => onEditDeck && onEditDeck(cur)}>✎ MODIFICA</button>
          </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {Array(2).fill(0).map((_, i) => (
            <span key={i}>SCHIERA · {mode.toUpperCase()} · {(opponent.faction || '').toUpperCase()} · MAPPA: {mapName.toUpperCase()} · {(opponent.name || '').toUpperCase()} · {' '}</span>
          ))}
        </div>
      </div>
